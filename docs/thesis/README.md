# Thesis Reference Documents

This directory contains the original PhD thesis and its tracking documents.

## Thesis Files

| File | Description | Size |
|------|-------------|------|
| [contribution-to-the-winding-theory.pdf](contribution-to-the-winding-theory.pdf) | Original thesis PDF — tracked via Git LFS | 16 MB |
| [contribution-to-the-winding-theory.md](contribution-to-the-winding-theory.md) | Full thesis text in Markdown (MinerU-extracted) | 294 KB |
| [contribution-to-the-winding-theory_mineru.json](contribution-to-the-winding-theory_mineru.json) | MinerU JSON metadata (full 200 pages) — tracked via Git LFS | 6 MB |

## Tracking Documents

| Document | Purpose |
|----------|---------|
| [paper_reference.md](../paper_reference.md) | Code ↔ Thesis chapter mapping table |
| [tracking.md](tracking.md) | Paper-code synchronization tracking (P0) |

## Thesis Information

- **Title:** Contribution to the winding theory: Introduction of a unified method for the treatment of winding topology
- **Author:** Mang Cai
- **University:** Technische Universität Braunschweig
- **Submission:** 23.11.2016
- **Defense:** 14.02.2017
- **Print Year:** 2017
- **Supervisor:** Prof. Dr.-Ing. W.-R. Canders
- **Second Reviewer:** Prof. Dr.-Ing. habil. Dr. h. c. K. Hameyer
